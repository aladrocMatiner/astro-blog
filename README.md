...
- `loki` + `alloy agent`: Grafana Alloy colecta logs Docker y los envía a Loki. El contenedor necesita acceso de solo lectura a `/var/lib/docker/containers` y `/var/run/docker.sock` para capturar los registros y exponer etiquetas de contenedor.
...
- Grafana Alloy reemplaza a Promtail y usa el socket de Docker con `observability/agent/config.alloy` para incluir labels como `container_name`, `image` y `container_id` en Loki.
